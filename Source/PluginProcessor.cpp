...
double WeatherSoundAudioProcessor::getTailLengthSeconds() const { return 0.0; }
...
int WeatherSoundAudioProcessor::getCurrentProgram() { return 0; }
...
void WeatherSoundAudioProcessor::changeProgramName(int index,
                                                   const String &newName) {}

//==============================================================================
void WeatherSoundAudioProcessor::prepareToPlay(double sampleRate,
                                               int samplesPerBlock) {
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool WeatherSoundAudioProcessor::isBusesLayoutSupported(
...
void WeatherSoundAudioProcessor::processBlock(AudioBuffer<float> &buffer,
  ...
  const float q = 0.707f * Decibels::decibelsToGain(
                               res); // Convert dB to linear gain. 0 dB = 0.707
...
void WeatherSoundAudioProcessor::setStateInformation(const void *data,
                                                     int sizeInBytes) {
  ...
  return {
      std::make_unique<
          AudioParameterFloat>( // why use make_unique? because the
                                ...
                                // ParameterLayout object, which is a vector of
                                // unique pointers to RangedAudioParameter
                                // objects. By using make_unique, we can create
                                // a new AudioParameterFloat object and
                                // automatically wrap it in a unique pointer,
                                // which is then added to the ParameterLayout
                                // vector.
          ParameterID{"cloudCoverage", 1}, "Cloud Coverage", 0.0f, 100.0f,
          50.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"humidity", 1},
                                            "Humidity", 0.0f, 100.0f, 50.0f),
      std::make_unique<AudioParameterFloat>(
          ParameterID{"temperature", 1}, "Temperature", -40.0f, 50.0f, 20.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"uvIndex", 1},
                                            "UV Index", 0.0f, 11.0f, 5.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"windSpeed", 1},
                                            "Wind Speed", 0.0f, 100.0f, 15.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"windDirection", 1},
                                            "Wind Direction", 0.0f, 360.0f,
                                            180.0f),
      std::make_unique<AudioParameterFloat>(
          ParameterID{"visibility", 1}, "Visibility", 0.0f, 296000.0f, 5.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"cloudCoverageMix", 1},
                                            "Cloud Coverage Mix", 0.0f, 100.0f,
                                            50.0f),
      std::make_unique<AudioParameterFloat>(
          ParameterID{"humidityMix", 1}, "Humidity Mix", 0.0f, 100.0f, 50.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"temperatureMix", 1},
                                            "Temperature Mix", 0.0f, 100.0f,
                                            50.0f),
      std::make_unique<AudioParameterFloat>(
          ParameterID{"uvIndexMix", 1}, "UV Index Mix", 0.0f, 100.0f, 50.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"windSpeedMix", 1},
                                            "Wind Speed Mix", 0.0f, 100.0f,
                                            50.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"windDirectionMix", 1},
                                            "Wind Direction Mix", 0.0f, 100.0f,
                                            50.0f),
      std::make_unique<AudioParameterFloat>(ParameterID{"visibilityMix", 1},
                                            "Visibility Mix", 0.0f, 100.0f,
                                            50.0f),
      std::make_unique<AudioParameterBool>(ParameterID{"bypass", 1}, "Bypass",
                                           false)};
}